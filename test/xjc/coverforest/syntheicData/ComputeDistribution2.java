...
public class ComputeDistribution2 {
	public static void main(String[] args) {
		System.out.println(ComputeDistribution2.class);
		}
...
/*	tree-1
	...
	p3:673,745,709,		
	p4:*/
	public static PartitionResult[]  computeCoverForestCentersDistribution(IDistanceHolder cd,int treeSize,int numCenter,CenterMeasureType cmt,int[][] bound) throws IOException, ClassNotFoundException{
		...
		if(cd.getClass()==CoverForestInstanceDataset.class)
			...
			CoverTreeInstanceDataset cd2=(CoverTreeInstanceDataset)cd;
...
/*	1-matchCount:0
	...
	*/
	public static HashMap<Integer, ArrayList<Integer>>  computeCoverForestTreeCentersPartitionOccupied(IDistanceHolder cd,int treeSize,int treeMax,int numCenter,CenterMeasureType cmt,int[][] bound) throws IOException, ClassNotFoundException{
		
		PartitionResult[] centerDistributionResults=computeCoverForestCentersDistribution(cd,treeSize,numCenter,cmt,bound);		
	...
	public static HashMap<Integer, ArrayList<Integer>>  computeCoverForestTreeCentersPartitionOccupied(CoverForest cf,int treeSize,int numCenter,CenterMeasureType cmt,int[][] bound) throws IOException, ClassNotFoundException{
		
		PartitionResult[] centerDistributionResults=computeCoverForestCentersDistribution(cf,treeSize,numCenter,cmt,bound);		
	...
	/*	1-matchRate:0.0
		...
		*/
	public static HashMap<Integer, Double>   computeCoverForestTreesPartitionOccupiedCountMap(IDistanceHolder cd,int treeSize,int treeMax,int numCenter,CenterMeasureType cmt,int[][] bound) throws IOException, ClassNotFoundException{
		...
		HashMap<Integer, ArrayList<Integer>> partitionOccupiedMap=computeCoverForestTreeCentersPartitionOccupied(cd,treeSize,treeMax,numCenter,cmt,bound);
...
/*			System.out.println(i+"-matchCount:"+th.size());
	...
	public static HashMap<Integer, Double>   computeCoverForestTreesPartitionOccupiedCountMap(File  pathDir,CoverForest cf,int[] treeSizes,int[] numCenters,CenterMeasureType cmt,int[][] bound) throws IOException, ClassNotFoundException{
		...
		for(int ni=0;ni<numCenters.length;ni++){
			...
			for(int ti=0;ti<treeSizes.length;ti++){
				...
				bw.write(treeSize+",");
				HashMap<Integer, ArrayList<Integer>> partitionOccupiedMap=computeCoverForestTreeCentersPartitionOccupied(cf,treeSize,numCenter,cmt,bound);
