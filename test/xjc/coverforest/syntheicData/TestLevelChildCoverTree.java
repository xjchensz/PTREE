...
public class TestLevelChildCoverTree {
	...
	public static void main(String[] args) throws Exception{
		...
		treeSize=10;int offset=2;treeSizeMax=30;int[]treeSizes=new int[(int)Math.ceil((double)(treeSizeMax-treeSize)/offset)];
		...
		int numCenter=5;
		int numCentetOffset=1; int numCenterMax=20;
		
 		
		File  savePathDir=new File(BuildResultStore.resultDir,"TreeCenter-PartitionOccupedResult/partitionOccupiedRate");
		...
		String resultDir=BuildResultStore.resultDir.getPath()+"/CoverTreeChildMap/syntheticData_[gaussianDataSize="+gaussianDataSize+"]";
		LevelChildCoverTree cTree=cd.getCoverTree();
		
		File saveFile=new File(resultDir,"covertree.ctr");
		saveCoverTree(saveFile,cTree);
		LevelChildCoverTree nCTree=new LevelChildCoverTree();
		readCoverTree(nCTree);
		System.out.println(cTree.maxLevel());
	...
	public static void readCoverTree(LevelChildCoverTree cTree)throws IOException{
		DataInputStream in = new DataInputStream(new FileInputStream(new File(BuildResultStore.resultDir, "covertree.ctr")));
		cTree.readFields(in);		
	}
	
	public static void saveCoverTree(File saveFile,LevelChildCoverTree cTree) throws IOException{
	...
	public static void saveLevelChildCoverTreeChildMap(String resultDir,LevelChildCoverTree cTree,double[][] synData) throws IOException{
		    ...
		    String childListFilePath=resultDir+"/tree-"+0;//BuildSuperStoreData.dir+"/result";
	...
	public static void saveLevelChildCoverForestChildMap(String resultDir,LevelChildCoverForest cf,NumericInstanceDistanceHolder nid)throws IOException{
		File resultDirFile=new File(resultDir);
		if(!resultDirFile.exists()) resultDirFile.mkdirs();
		for(int i=0;i<cf.getTreeSize();i++){
			saveLevelChildCoverTreeChildMap(resultDir+"/tree-"+i, cf.getCoverTree(i), null);
		}
	}
	...
	public static void computeCoverForestLevelDensity(LevelChildCoverTreeInstanceDataset cd,CoverForest cf,int treeSizeMax) throws IOException{
		OrderedIntMap map=new OrderedIntMap();
	...
	public static void computeCoverForestRealLevelDensity(LevelChildCoverTreeInstanceDataset cd,CoverForest cf,int treeSizeMax) throws IOException{
		OrderedIntMap map=new OrderedIntMap();
		LevelChildCoverTree cts[];
	    ...
	    for(int ti=0;ti<cts.length;ti++){
	    	cts[ti].update();
	    	cts[ti].update();
	    	System.out.println("cover tree_"+ti+" realLevelDensity");
	    	int maxLevel=cts[ti].maxLevel();int minLevel=maxLevel-10;
		    for(int level=maxLevel;level>minLevel;level--){			    	
					map.clear();
					realLevelDensity(tStringBuilder,cd,level, map);
