...
public class ClassicCoverTree<E> {
	 ...
	 */
	public ClassicCoverTree(double base, int maxMinLevel) {
		this.base = base;
	...
	void insertAtRoot(E element, double[] point) {
		// inserts the point above the root by successively increaasing the
		...
		while (dist > Math.pow(base, maxLevel)) {
			Node<E> newRoot = new Node<E>(null, rootNode.element,
					rootNode.point);
	...
	/**
	 * Insert a point into the tree. If the tree size is greater than k the
	 * lowest cover will be removed as long as it does not decrease tree size
	 * below k.
	 * 
	 * @param point
	 */
	public boolean insert(E element, double[] point, int k) {
		...
		if (maxLevel - minLevel > 2) {
			// remove lowest cover if the cover before has a sufficient number
			// of nodes
	...
	public boolean insert(E element, double[] point) {
		...
		while (true) {
			...
			// if the children of the coverset are further away the 2^level then
			// an element of the
		...
		// if the point is a sibling of the root node, then the cover of the
		// root node is increased
		if (parent == null) {
			insertAtRoot(element, point);
			return true;
		}
		if (parentLevel - 1 < minLevel) {
			// if the maximum size is reached and this would only increase the
			// depth of the tree then stop
		...
		// record distance to parent node and add to the sorted set of nodes
		// where distance is used for sorting (needed for removal)
	...
	List<Node<E>> removeNodes(int numCenters) {
		...
		if (missing < 0)
			System.err.println("Error: negative missing=" + missing
					+ " in coverset");
		// sucessively pick the node with the largest distance to the coverset
		// and add it to the coverset
		LinkedList<Node<E>> candidates = new LinkedList<Node<E>>();
		for (Node<E> n : coverset)
			for (Node<E> n2 : n.getChildren())
				if (n.point != n2.point)
					candidates.add(n2);
		// only add candidates when the coverset is yet smaller then the number
		// of desired centers
		if (coverset.size() < numCenters) {
			// compute the distance of all candidates to their parents and
			// uncles
			for (Node<E> n1 : candidates) {
				...
				if (minDist == Double.POSITIVE_INFINITY)
					System.err
							.println("Error: Infinite distance in k centers computation.");
			}
			do {
	...
	/**
	 * Retrieve the elemnet from the tree that is nearest to the given point
	 * with respect to the Euclidian distance.
	 * 
	 * @param point
	 * @return
	 */
	 ...
	 * points to the tree. Otherwise, it will return the cover one level above
