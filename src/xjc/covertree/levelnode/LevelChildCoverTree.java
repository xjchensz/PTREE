...
public class LevelChildCoverTree implements IWritable {
	...
	private ILevelNodeCreator m_NodeCreator;
	...
	private ArrayList<ILevelNode> tmpNodes1 = new ArrayList<ILevelNode>();
	private ArrayList<ILevelNode> tmpNodes2 = new ArrayList<ILevelNode>();
	private ArrayList<ILevelNode> tmpNodes3 = new ArrayList<ILevelNode>();
	private ArrayList<ILevelNode> tmpNodes4 = new ArrayList<ILevelNode>();
	private OrderedDoubleMap tmpMap = new OrderedDoubleMap();
	private OrderedIntArraySet os = new OrderedIntArraySet();

	public LevelChildCoverTree(){}
	public LevelChildCoverTree(LevelChildCoverTree ct, IDistanceHolder distanceHolder) {
	...
	private LevelChildCoverTree(IDistanceHolder distanceHolder) {
		m_DistanceHolder = distanceHolder;
		tmpMap.setEmptyValue(-1);
	}
	 ...
	 * below.
	 */
	LevelChildCoverTree(ILevelNodeCreator creator, IDistanceHolder distanceHolder) {
	 ...
	 */
	LevelChildCoverTree(ILevelNodeCreator creator, IDistanceHolder distanceHolder, double base, int maxNumLevels, int minNumLevels) {
	...
	private boolean insertAtRoot(int instance, double distance) {
		...
		if (rootNode.numChildren() == 0) {
			...
			while (distance > disThreshold) {
				newRoot = m_NodeCreator.createParentNode(rootNode,maxLevel+1);
				decNodes(maxLevel);
				rootNode = newRoot;
				incNodes(++maxLevel);
				disThreshold *= base;
		...
		m_NodeCreator.createChildNode(rootNode, instance,maxLevel-1);
	 ...
	 * @param point
	 */
	public boolean insert(int instance) {

		// if this is the first node make it the root node
		if (rootNode == null) {
			rootNode = m_NodeCreator.createRootNode(instance,maxLevel);
			incNodes(maxLevel);
			minLevel = maxLevel;
			return true;
		}
		...
		while (true) {
			...
			for (int i = tmpNodes1.size() - 1, j; i >= 0 && !zerodistance; i--) {
				...
				ILevelNode parent=tmpNodes1.get(i);
				distance = m_DistanceHolder.distance(parent.getInstance(), instance);
				if (distance == 0) {
					parent.attach(instance);
					zerodistance = true;
					break;
				}
				tmpMap.put(parent.getInstance(), distance);
				if (distance < distanceThreshold) {
					tmpNodes2.add(parent);
				}
				
				if (children != null) {
					for (j = children.size() - 1; j >= 0; j--) {
						child = children.get(j);
						if(child.getLevel()!=level) continue;
			...
			if (tmpNodes2.isEmpty()) {
				...
				ILevelNode nearestNode = null;
				double minDist = Double.MAX_VALUE;
				double dist;
				for (int i = tmpNodes1.size() - 1; i >= 0; i--) {
					dist = tmpMap.get(tmpNodes1.get(i).getInstance());
					if (dist < minDist) {
				...
				m_NodeCreator.createChildNode(nearestNode, instance,level);
				incNodes(level);
				break;
			}
	...
	public int[] getKNearestNeighbor(int instance, int k) {
		ILevelNode[] knnNodes = getKNearestNode(instance, k);
		int[] kNNs = new int[k];
		int nodeLength = 0;
		int[] nodeAttached;
	 ...
	 * @return
	 */
	public ILevelNode[] getKNearestNode(int instance, int k) {
		if (k < 1)
			return null;
		...
		List<ILevelNode> children;// children为锟接节碉拷锟絠d锟斤拷锟斤拷
		...
		// ArrayList<ILevelNode> knnNodes=new ArrayList<ILevelNode>();

		ILevelNode[] knnNodes = new ILevelNode[k];
		int[] kNNs = new int[k];
		for (int i = 0; i < k; i++) {
	...
	public double getKNNRadius(int instance, int k) {
		...
		int maxKIns = (getKNearestNeighbor(instance, k+1))[k];
	...
	public int[] getKLevelDensityCenters(int numCenters) {
		...
		//levelDensity2(new StringBuilder(),level, densities);
		//realLevelDensity(new StringBuilder(),level, densities);
	...
	public int[] getKCentralityCenters(int numCenters) {
		...
		Centrality[] dn = centrality(level, numCenters);
	...
	public Centrality[] getKCentralityCentersOnLevelDensity(int numCenters) {
		...
		// get k level density candidate centers
		
		
		Centrality[] dn = centrality(centers,numCenters);
...
/*		int[] results = new int[numCenters < dn.length ? numCenters : dn.length];
	...
	public Centrality[] getKCentralityCenterNodePairs(int numCenters) {
		...
		Centrality[] dn = centrality(level, numCenters);// get all nodes'
	...
	public Centrality[] centrality(int level, int k) {
		...
		ILevelNode node;
		ArrayList<ILevelNode> candidateNodes = new ArrayList<ILevelNode>(tmpNodes3);
		// compute and sort knnradius
	...
	public int[] getKcenters(int numClusters, CenterMeasureType cmt) {
		...
		case CENTRALITY:
		default:
			return getKCentralityCenters(numClusters);
		}
	}
	 
	//this is temporary-used code for CoverTree root
	public ILevelNode getRootNode(){
		return rootNode;
	}
	public IDistanceHolder getDistanceHolder(){return m_DistanceHolder;}
...
/*			tmp = tmpNodes1;
	...
	public ArrayList<ILevelNode> getLevelChildNodes(int level){
		LevelChildCoverTree cTree=this;
	...
	//	CoverTreeInstanceDataset m_DistanceHolder=ctd;//cTree.getDistanceHolder();
		...
		ArrayList<ILevelNode> tNodes2 = new ArrayList<ILevelNode>();
		ArrayList<ILevelNode> tNodes3 = new ArrayList<ILevelNode>();
		OrderedDoubleMap tmpMap = new OrderedDoubleMap();
		OrderedIntArraySet os = new OrderedIntArraySet();
		tNodes2.clear();
		tNodes3.clear();
		tNodes3.add(rootNode);
		...
		for (int l = maxLevel - 1, j, k; l >= level; l--) {
			tNodes2.clear();
			for (j = tNodes3.size() - 1; j >= 0; j--) {
				children = tNodes3.get(j).getChildren();
				if (children != null) {
					for (k = children.size() - 1; k >= 0; k--) {
						child = children.get(k);
						
						if(child.getLevel()==l)	tNodes2.add(child);
					}
				}
			}
			tNodes3.addAll(tNodes2);
		}

		os.clear();

		//System.out.println("level-"+level+":");
		for (int i = 0; i < tNodes3.size(); i++) {
			os.add(tNodes3.get(i).getInstance());
			//s.append(tmpNodes3.get(i).getInstance()+",");s.append("\n");
			//System.out.print(tmpNodes3.get(i).getInstance()+",");
		}
	...
	public  void realLevelDensity(StringBuilder s,int level, OrderedIntMap map) {
		s.append("level-"+level+":");s.append("\n");
		System.out.println("level-"+level+":");
		LevelChildCoverTree cTree=this;
		double base=cTree.getBase();
...
/*			tmp = tmpNodes1;
		...
		for(int i=0;i<os.size();i++){
			...
			for(int j=0;j<m_DistanceHolder.size();j++){
				if(m_DistanceHolder.distance(j, ins)<=threshold)count++;
			}
			map.put(ins, count);
		}
	...
	public void readFields(DataInput in) throws IOException {
		...
		try {
			m_NodeCreator = (ILevelNodeCreator) Class.forName(in.readUTF()).newInstance();
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof LevelChildCoverTree)) {
			return false;
		}
		LevelChildCoverTree ct = (LevelChildCoverTree) obj;
	...
	public static LevelChildCoverTree read(DataInput in, IDistanceHolder distanceHolder) throws IOException {
		LevelChildCoverTree ct = new LevelChildCoverTree(distanceHolder);
		ct.readFields(in);
		return ct;
	}
